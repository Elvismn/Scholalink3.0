...
const Parents = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Use EXACT SAME normalizeArray as Departments.jsx
  ...
  const filteredParents = useMemo(() => {
    ...
    return parents.filter(parent => {
      ...
      const parentName = `${parent.user?.profile?.firstName || ''} ${parent.user?.profile?.lastName || ''}`.toLowerCase()
  ...
  const openEditModal = (parent) => {
    ...
    // NOTE: Changed from parent.user?.firstName to parent.user?.profile?.firstName
    setFormData({
      userId: parent.user?._id || '',
      children: Array.isArray(parent.children) 
        ? parent.children.map(child => child._id || child)
        : [],
      address: parent.address || { street: '', city: '', state: '', zipCode: '' },
      emergencyContact: parent.emergencyContact || { name: '', relationship: '', phone: '' }
    })
  ...
  return (
    <div className="p-6">
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center space-y-4 lg:space-y-0 mb-6">
        <div className="text-center lg:text-left">
          ...
          <p className="text-gray-600">
            {searchTerm ? `Showing ${filteredParents.length} of ${parents.length} parents` : 'Manage parent accounts and child associations'}
          </p>
        </div>
          ...
          <div className="py-12 text-center">
            ...
            {!searchTerm && (
              <Button onClick={openCreateModal} className="flex items-center gap-2 mx-auto">
                <Plus className="w-4 h-4" /> Add Your First Parent
              </Button>
            )}
            {searchTerm && (
                ...
                <div key={parent._id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <h3 className="font-semibold text-gray-900 truncate">
                            {parent.user?.profile?.firstName || 'Unknown'} {parent.user?.profile?.lastName || ''}
                          </h3>
                    ...
                    <div className="flex space-x-1 flex-shrink-0 ml-2">
                      <button onClick={() => openEditModal(parent)} className="p-1 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded" title="Edit">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(parent._id)} className="p-1 text-red-600 hover:text-red-800 hover:bg-red-50 rounded" title="Delete">
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    ...
                    {parent.address && (parent.address.street || parent.address.city) && (
                      <div className="flex items-start gap-2">
                        ...
                        <span className="line-clamp-2">
                          {[parent.address.street, parent.address.city, parent.address.state, parent.address.zipCode].filter(Boolean).join(', ')}
                        </span>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span><strong>Children:</strong> {childrenCount}</span>
                    </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingParent(null); resetForm(); }} title={editingParent ? 'Edit Parent' : 'Add New Parent'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700">Select Parent User Account *</label>
            <select required value={formData.userId} onChange={(e) => setFormData({...formData, userId: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" disabled={editingParent !== null}>
            ...
            <p className="text-xs text-gray-500">
              {editingParent ? 'User account cannot be changed after creation' : 'Select the user account this parent profile will be linked to'}
            </p>
          </div>

          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 mb-2">Address</label>
            <Input placeholder="Street Address" value={formData.address.street} onChange={(e) => setFormData({...formData, address: {...formData.address, street: e.target.value}})} />
            <div className="grid grid-cols-3 gap-2">
              ...
              <Input placeholder="Zip Code" value={formData.address.zipCode} onChange={(e) => setFormData({...formData, address: {...formData.address, zipCode: e.target.value}})} />
            </div>
          </div>

          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 mb-2">Emergency Contact</label>
            <Input placeholder="Contact Name" value={formData.emergencyContact.name} onChange={(e) => setFormData({...formData, emergencyContact: {...formData.emergencyContact, name: e.target.value}})} />
            <div className="grid grid-cols-2 gap-2">
              <Input placeholder="Relationship" value={formData.emergencyContact.relationship} onChange={(e) => setFormData({...formData, emergencyContact: {...formData.emergencyContact, relationship: e.target.value}})} />
              <Input placeholder="Phone Number" type="tel" value={formData.emergencyContact.phone} onChange={(e) => setFormData({...formData, emergencyContact: {...formData.emergencyContact, phone: e.target.value}})} />
            </div>
          </div>
                ...
                <div className="space-y-2">
                  {students.map(student => (
                    <label key={student._id} className="flex items-center space-x-3 cursor-pointer hover:bg-gray-50 p-2 rounded">
                      <input type="checkbox" checked={formData.children.includes(student._id)} onChange={(e) => {
                        if (e.target.checked) {
                          setFormData({...formData, children: [...formData.children, student._id]})
                        } else {
                          setFormData({...formData, children: formData.children.filter(id => id !== student._id)})
                        }
          ...
          <div className="pt-4 flex justify-end gap-3 border-t">
            <Button variant="secondary" type="button" onClick={() => { setIsModalOpen(false); setEditingParent(null); resetForm(); }}>
              Cancel
            </Button>
